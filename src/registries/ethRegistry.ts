...
import abi from './ethRegistryAbi.js'
import nftAbi from './nftContractAbi.js'
...
export class EthRegistry implements Registry {
            ...
            const result = Object.fromEntries(
                modules.map((modules, i) => {
                    ...
                    const mis = modules.map((m, j) =>
                        this._convertFromEthMi({ ...m, owner: owners[i][j] })
                    )
                        ...
                        contract
                            ...
                            .then((resp) => ({
                                items: resp.modules.map((module, i) => ({
                                    ...
                                    versions: [
                                        this._convertFromEthVi(
                                            resp.lastVersions[i],
                                            resp.modules[i].moduleType,
                                            {
    ...
    public async addModule(module: ModuleInfo, version: VersionInfo): Promise<any> {
        const contract = await this._contractPromise
            ...
            return (
                (dependencies &&
                    Object.entries(dependencies).map(([k, v]) => ({
                        name: k,
                        branch: 'default',
                        version: this._convertToEthVersion(
